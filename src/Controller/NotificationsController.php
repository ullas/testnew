...
class NotificationsController extends AppController
{
    ...
    public function index()
    {
        $this->loadModel('CreateConfigs');
         $configs=$this->CreateConfigs->find('all')->where(['table_name' => 'Notifications'])->order(['"order"' => 'ASC'])->toArray();
		 $this->loadModel('Usersettings');
		 $usersettings=$this->Usersettings->find('all')->where(['user_id' => $this->loggedinuser['id']])->where(['module' => 'Notifications'])->where(['key' => 'INIT_VISIBLE_COLUMNS_NOTIFICATIONS'])->toArray();
         if(isset($usersettings[0]['value'])){
		    ...
		    $usersettings=$this->Globalusersettings->find('all')->where(['module' => 'Notifications'])->where(['key' => 'INIT_VISIBLE_COLUMNS_NOTIFICATIONS'])->toArray();
            $this->set('usersettings',$usersettings);
	...
	public function updateSettings()
{
	...
	$count = $userSettings->find('all')
	   ->where(['key' => 'INIT_VISIBLE_COLUMNS_NOTIFICATIONS'])
	...
	$this->response->body($res);
	   ...
	   ->values(
	       ['key'=>'INIT_VISIBLE_COLUMNS_NOTIFICATIONS',
	        'value'=>$columns,
...
public function ajaxdata() {
       ...
       $this->loadModel('CreateConfigs');
       $dbout=$this->CreateConfigs->find('all')->where(['table_name' => 'Notifications'])->order(['"order"' => 'ASC'])->toArray();
        ...
        $output =$this->Datatable->getView($fields,['Timepolicies', 'Subscriptions', 'Customers'],$usrfiter);
    ...
    public function view($id = null)
    {
        $notification = $this->Notifications->get($id, [
            'contain' => ['Timepolicies', 'Subscriptions']
        ]);
		...
		if(isset($data)){
		   foreach($data as $key =>$value){
			    ...
			    if(count($itemna)== 2 && $itemna[0]=='chk'){
					...
					$record = $this->Notifications->get($value);
					
					 if($record['customer_id']== $this->loggedinuser['customer_id']) {
					 	
						   if ($this->Notifications->delete($record)) {
					           $sucess= $sucess | true;
					        } else {
				...
				if($sucess){
					$this->Flash->success(__('Selected Notifications has been deleted.'));
				}
		        if($failure){
