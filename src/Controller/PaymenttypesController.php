 ...
 *
 * @property \App\Model\Table\PaymenttypesTable $Paymenttypes * @property \App\Controller\Component\DatatableComponent $Datatable */
class PaymenttypesController extends AppController
{
    ...
    public function index()
    {
        ...
        */
         ...
         $configs=$this->CreateConfigs->find('all')->where(['table_name' => 'Paymenttypes'])->order(['id' => 'ASC'])->toArray();
         ...
         $this->loadModel('CreateConfigs');
         $configs=$this->CreateConfigs->find('all')->where(['table_name' => 'Paymenttypes'])->order(['"order"' => 'ASC'])->toArray();
		 $this->loadModel('Usersettings');
		 $usersettings=$this->Usersettings->find('all')->where(['user_id' => $this->loggedinuser['id']])->where(['module' => 'Workorders'])->where(['key' => 'INIT_VISIBLE_COLUMNS_Paymenttypes'])->toArray();
...
public function ajaxdata() {
       ...
       $this->loadModel('CreateConfigs');
       $dbout=$this->CreateConfigs->find('all')->where(['table_name' => 'Paymenttypes'])->order(['id' => 'ASC'])->toArray();
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $paymenttype['customer_id']=$this->currentuser['customer_id'];
          ...
          $customers = $this->Paymenttypes->Customers->find('list', ['limit' => 200])->where("id=".$this->loggedinuser['customer_id']);
      
        
                $this->set(compact('paymenttype', 'customers'));
        $this->set('_serialize', ['paymenttype']);
    }
    ...
    public function edit($id = null)
    {
        ...
        $customers = $this->Paymenttypes->Customers->find('list', ['limit' => 200]);
        $this->set(compact('paymenttype', 'customers'));
        $this->set('_serialize', ['paymenttype']);
    }
