<section class="content-header">
  <h1>
     <?= h($shipmenttype->name) ?> Details
  </h1>
  <ol class="breadcrumb">
  	<li><a href="#"><i class="fa fa-dashboard"></i>Home</a></li>
   
    <li><a href="/shipmenttype/"> shipmenttype</a></li>
...
<section class="content">
  ...
  <div class="col-md-12">
  	...
  	<div class="box box-primary">
        ...
        <tr>
            <th><?= __('Customer') ?></th>
            <td><?= $shipmenttype->has('customer') ? $this->Html->link($shipmenttype->customer->name, ['controller' => 'Customers', 'action' => 'view', $shipmenttype->customer->id]) : '' ?></td>
        </tr>
        <tr>
            <th><?= __('Id') ?></th>
    ...
    <div class="row">
        ...
        <h4><?= __('Related Cargotrips') ?></h4>
        <?php if (!empty($shipmenttype->cargotrips)): ?>
        </div>
  		<div class="box-body">
  		<table class="table table-hover">
            <tr>
            ...
            <?php foreach ($shipmenttype->cargotrips as $cargotrips): ?>
            <tr>
                <td><?= h($cargotrips->id) ?></td>
                <td><?= h($cargotrips->shipmenttype_id) ?></td>
                <td><?= h($cargotrips->packageweight) ?></td>
                <td><?= h($cargotrips->packagevalue) ?></td>
                <td><?= h($cargotrips->packagevolume) ?></td>
                <td><?= h($cargotrips->modeoftransport_id) ?></td>
                <td><?= h($cargotrips->lrnumber) ?></td>
                <td><?= h($cargotrips->flightnum) ?></td>
                <td><?= h($cargotrips->trainnum) ?></td>
                <td><?= h($cargotrips->customer_id) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'Cargotrips', 'action' => 'view', $cargotrips->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'Cargotrips', 'action' => 'edit', $cargotrips->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Cargotrips', 'action' => 'delete', $cargotrips->id], ['confirm' => __('Are you sure you want to delete # {0}?', $cargotrips->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
    ...
    <div class="row">
        ...
        <?php if (!empty($shipmenttype->deiveryorders)): ?>
        </div>
  		<div class="box-body">
  		<table class="table table-hover">
            <tr>
            ...
            <?php foreach ($shipmenttype->deiveryorders as $deiveryorders): ?>
            <tr>
    ...
    <div class="row">
        ...
        <?php if (!empty($shipmenttype->pickupdeiveryorders)): ?>
        </div>
  		<div class="box-body">
  		<table class="table table-hover">
            <tr>
            ...
            <?php foreach ($shipmenttype->pickupdeiveryorders as $pickupdeiveryorders): ?>
            <tr>
    ...
    <div class="row">
        ...
        <?php if (!empty($shipmenttype->pickuporders)): ?>
        </div>
  		<div class="box-body">
  		<table class="table table-hover">
            <tr>
            ...
            <?php foreach ($shipmenttype->pickuporders as $pickuporders): ?>
            <tr>
