<section class="content-header">
  <h1>
     <?= h($simcard->name) ?> Details
  </h1>
  <ol class="breadcrumb">
  	<li><a href="#"><i class="fa fa-dashboard"></i>Home</a></li>
   
    <li><a href="/simcard/"> simcard</a></li>
...
<section class="content">
  ...
  <div class="col-md-12">
  	...
  	<div class="box box-primary">
        ...
        <tr>
            <th><?= __('Customer') ?></th>
            <td><?= $simcard->has('customer') ? $this->Html->link($simcard->customer->name, ['controller' => 'Customers', 'action' => 'view', $simcard->customer->id]) : '' ?></td>
        </tr>
        <tr>
            <th><?= __('Id') ?></th>
            <td><?= $this->Number->format($simcard->id) ?></td>
        </tr>
        <tr>
            <th><?= __('Billdayofmonth') ?></th>
            <td><?= $this->Number->format($simcard->billdayofmonth) ?></td>
        </tr>
        <tr>
            <th><?= __('Duedayofmonth') ?></th>
            <td><?= $this->Number->format($simcard->duedayofmonth) ?></td>
        </tr>
        <tr>
            <th><?= __('Lastdatewithfine') ?></th>
    ...
    <div class="row">
        ...
        <?php if (!empty($simcard->devices)): ?>
  		...
  		<table class="table table-hover">
            ...
            <?php foreach ($simcard->devices as $devices): ?>
            <tr>
                <td><?= h($devices->id) ?></td>
