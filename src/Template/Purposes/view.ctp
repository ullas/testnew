<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Purpose'), ['action' => 'edit', $purpose->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Purpose'), ['action' => 'delete', $purpose->id], ['confirm' => __('Are you sure you want to delete # {0}?', $purpose->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Purposes'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Purpose'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Vehicles'), ['controller' => 'Vehicles', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Vehicle'), ['controller' => 'Vehicles', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="purposes view large-9 medium-8 columns content">
    <h3><?= h($purpose->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            <th><?= __('Id') ?></th>
            <td><?= $this->Number->format($purpose->id) ?></td>
        </tr>
        <tr>
            <th><?= __('Customer Id') ?></th>
            <td><?= $this->Number->format($purpose->customer_id) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Vehicles') ?></h4>
        <?php if (!empty($purpose->vehicles)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($purpose->vehicles as $vehicles): ?>
            <tr>
                <td><?= h($vehicles->id) ?></td>
