<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Symbol'), ['action' => 'edit', $symbol->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Symbol'), ['action' => 'delete', $symbol->id], ['confirm' => __('Are you sure you want to delete # {0}?', $symbol->id)]) ?> </li>
...
<div class="symbols view large-9 medium-8 columns content">
    <h3><?= h($symbol->name) ?></h3>
    ...
    <div class="row">
        <h4><?= __('Symbol') ?></h4>
        <?= $this->Text->autoParagraph(h($symbol->symbol)); ?>
    </div>
    <div class="related">
        <h4><?= __('Related Vehicles') ?></h4>
        <?php if (!empty($symbol->vehicles)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($symbol->vehicles as $vehicles): ?>
            <tr>
                <td><?= h($vehicles->id) ?></td>
